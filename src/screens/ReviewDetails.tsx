  ...
  const [comments, setComments] = useState<Comment[]>([
    {
      ...
      text: 'This review perfectly captures my thoughts about the book!',
      date: '1 day ago'
    },
    {
      id: 2,
      username: 'literature_fan',
      text: 'I disagree with some points but appreciate the thorough analysis.',
      date: '3 days ago'
    }
  ]);
  const [currentReview, setCurrentReview] = useState<Review>(reviewData);
  ...
  return (
    <View className="flex-1 bg-[#f0fdfa]">
      ...
      <View className="flex-1">
        ...
        >
          {/* Review Card - Now properly included */}
          <View className="bg-white rounded-2xl p-6 mx-4 my-4 shadow-lg shadow-[#0d9488]/20">
            ...
            <Text className="text-base text-[#475569] leading-6 mb-6">
            ...
            <View className="flex-row justify-between items-center border-t border-[#e2e8f0] pt-4">
              <View className="flex-row">
                ...
                >
                  ...
                  />
                  <Text className={`ml-1 text-base ${currentReview.upvoted ? 'text-[#ef4444] font-semibold' : 'text-[#64748b]'}`}>
                ...
                >
                  ...
                  />
                  <Text className={`ml-1 text-base ${currentReview.downvoted ? 'text-[#ef4444] font-semibold' : 'text-[#64748b]'}`}>
          ...
          <View className="bg-white rounded-2xl p-6 mx-4 my-2 shadow-md shadow-[#0d9488]/10">
            ...
            {comments.map((comment, index) => (
              <View 
                key={comment.id} 
                className={`pb-4 ${index !== comments.length - 1 ? 'border-b border-[#e2e8f0] mb-4' : ''}`}
              >
                <View className="flex-row justify-between items-start mb-2">
                  <Text className="text-sm font-semibold text-[#0d9488]">@{comment.username}</Text>
                  <Text className="text-xs text-[#94a3b8]">{comment.date}</Text>
                </View>
                <Text className="text-sm text-[#475569] leading-5">{comment.text}</Text>
              </View>
            ))}
          </View>
        </ScrollView>
        ...
        >
          <View className="bg-white rounded-full p-1 shadow-lg shadow-black/20 flex-row items-center">
            ...
            />
            <TouchableOpacity 
              className="bg-[#0d9488] p-3 rounded-full"
